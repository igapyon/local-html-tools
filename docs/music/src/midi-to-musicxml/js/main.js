...
function buildMeasuresFromMonophonicTrack(notes, ppqn, meter, keyFifths) {
    ...
    for (const note of notes) {
        ...
        if (start > cursor) {
            const inserted = pushDurationToken(measures, cursor, start - cursor, ticksPerMeasure, ppqn, null, measureAccidentalStates, keySignatureStepAlter);
            restCount += inserted;
            cursor = start;
        }
        const insertedNotes = pushDurationToken(measures, cursor, duration, ticksPerMeasure, ppqn, note.noteNumber, measureAccidentalStates, keySignatureStepAlter);
...
function pushDurationToken(measures, startTick, durationTick, ticksPerMeasure, ppqn, midiNoteNumber, measureAccidentalStates, keySignatureStepAlter) {
    ...
    while (remaining > 0) {
        ...
        if (midiNoteNumber === null) {
            ...
            measures[measureIndex].push(buildNoteToken(midiNoteNumber, chunk, ppqn, keySignatureStepAlter, state));
        }
        inserted += 1;
        tick += chunk;
        remaining -= chunk;
...
function buildNoteToken(midiNoteNumber, durationTick, ppqn, keySignatureStepAlter, measureAccidentalState) {
    ...
    const accidentalText = resolveAccidentalTextForMeasure(pitch.step, pitch.octave, alterNumber, keySignatureStepAlter, measureAccidentalState);
    return {
        isRest: false,
        step: pitch.step,
        alter: pitch.alter,
...
function readU32(reader) {
    ...
    const value = (reader.bytes[reader.offset] * 16777216) +
