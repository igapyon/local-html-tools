...
${getSoftHallucinationPreventionInstruction()}`
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => `今までのセッションでの会話を別の生成AIに引継 (KT) したいです。受け手が生成AIであることを前提に、状況を再現しやすい引き継ぎテキストを Markdown 形式で生成してください。特に、作業の目的、前提条件、決まったこと、未確定事項、未完了事項、次に見るべきファイルや論点、注意点を明確に区別して整理してください。可能であれば、受け手の生成AIがすぐに再開しやすい順番で並べてください。
...
${getSoftHallucinationPreventionInstruction()}`
  ...
  },
  {
    ...
    buildBody: (commitId: string) => commitId
      ? `対象コミット ${commitId} における変更内容について、GitHub PR 用のタイトルと本文を Markdown 形式で作成してください。PR 本文には、少なくとも概要、主な変更点、テストまたは確認内容、必要であれば注意点やリスクを含めてください。変更内容に基づかない推測は避け、分からない点は断定しないでください。
...
${getMarkdownFenceInstruction()}`
  ...
  },
  {
    ...
    requiresCommitId: true,
    buildBody: (commitId: string) => commitId
      ? `${commitId} より後に行われた変更（${commitId} 自体の変更内容は除外する）について、GitHub Release 用のタイトルと本文を Markdown 形式で作成してください。利用者や読者が把握しやすいように、概要、主な変更点、必要であれば注意点や既知の制約を整理してください。変更内容に基づかない推測は避け、分からない点は断定しないでください。
...
${getMarkdownFenceInstruction()}`
  ...
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => `今回の事象について、再発時に早く気づける小さく単純な再発防止テストを検討してください。ただし、テストは無条件に増やすのではなく、既存のテスト基盤で無理なく追加でき、保守コストに見合い、今回の事象の検知に実際に寄与する場合に限って追加してください。

重い統合テスト、新規テスト基盤の導入、大量のモック準備が必要なもの、効果に対して複雑すぎるもの、実装詳細に強く結びつき壊れやすいだけのものは避けてください。追加する場合は、できるだけ小さく、意図が明確で、その事象の再発を直接検知できる形にしてください。

追加しない場合は、単に見送るのではなく、なぜ追加価値が低いのか、どの依存や構造が障害になっているのか、どの粒度なら現実的かを説明してください。必要であれば、コードコメント、ガード、入力検証、簡易確認手順、別の小さな検知方法など、テスト以外の再発防止策も示してください。
