function appendStrictInstruction(body: string): string {
  return `${body}\n\n${getStrictHallucinationPreventionInstruction()}`;
}

function appendSoftInstruction(body: string): string {
  return `${body}\n\n${getSoftHallucinationPreventionInstruction()}`;
}

function appendStrictMarkdownFenceInstruction(body: string): string {
  return `${body}\n\n${getStrictHallucinationPreventionInstruction()}\n\n${getMarkdownFenceInstruction()}`;
}

function appendSoftMarkdownFenceInstruction(body: string): string {
  return `${body}\n\n${getSoftHallucinationPreventionInstruction()}\n\n${getMarkdownFenceInstruction()}`;
}
...
const popularPromptDefinitions = [
  {
    ...
    buildBody: () => appendSoftInstruction("与えられた内容から、暗黙の前提も含めて前提条件を抽出してください。何が成立している前提で話が進んでいるのか、どの前提が崩れると結論や作業方針に影響するのかが分かるように整理してください。必要に応じて、明示されている前提と暗黙の前提を分けて示してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容から、制約条件や守るべき境界を抽出してください。時間、コスト、依存関係、互換性、禁止事項、作業範囲など、結論や実装方針を縛る条件を整理し、重要度が高いものから分かるように示してください。前提条件と混ざりやすい場合は、その違いも区別して説明してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容について、見落としやすいリスクを列挙してください。仕様、運用、保守、依存関係、テスト、認識齟齬などの観点から、問題になりそうな点を幅広く洗い出してください。まだ重大度評価までは厳密にしなくてよいので、抜け漏れを減らすことを優先してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容について、見落としリスクを重大度順に整理してください。発生時の影響の大きさ、起こりやすさ、発見のしにくさを考慮し、どのリスクを先に潰すべきかが分かるように並べてください。可能であれば、各リスクの理由や対策候補も簡潔に添えてください。")
  },
  {
  ...
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた選択肢について、比較表を作成してください。少なくとも各選択肢の特徴、メリット、デメリット、向いている場面が比較できるように整理してください。表形式または Markdown テーブルで、判断材料として使いやすい形にまとめてください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた選択肢について、メリットとデメリットが見比べやすい比較表を作成してください。結論を急がず、各案の良い点と弱い点が対称に読める形で整理してください。必要に応じて、判断時に重視すべき観点も列として加えてください。")
  },
  {
  ...
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendStrictInstruction("与えられた内容から、決定事項だけを抽出してください。未決事項や背景説明とは分けて、何がすでに決まっているのかが一目で分かるように整理してください。必要であれば、各決定事項に簡潔な理由や前提も添えてください。")
  },
  {
  ...
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftMarkdownFenceInstruction("与えられた内容を Markdown の表形式で出力してください。比較、分類、一覧化に向くように列を整理し、表だけで主要な情報が読み取れるようにまとめてください。必要に応じて、列名や並び順も読みやすさを優先して調整してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた選択肢を比較したうえで、推奨案を 1 つ選んでください。単に結論だけを述べるのではなく、なぜその案を勧めるのか、他案より優先する理由が分かるように整理してください。必要であれば、推奨しない案の理由も簡潔に添えてください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた選択肢について、トレードオフを整理したうえで推奨案を示してください。コスト、速度、保守性、理解しやすさなど、何を優先するとどの案が有利になるのかが分かるように説明してください。結論は、どの価値観を重視した推奨なのかも含めて示してください。")
  },
  {
  ...
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた状況から、次の一手を 3 件に絞ってください。やれることを網羅するのではなく、今この順で進めると効果が高いものに絞り込み、優先順位つきで示してください。必要であれば、なぜその 3 件を選んだのかも簡潔に説明してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた候補や作業を、実行順に並べてください。依存関係、着手しやすさ、効果の大きさ、先に確認すべき前提を考慮して、どの順で進めるのが自然かが分かるように整理してください。必要に応じて、後回しにする理由も簡潔に添えてください。")
  },
  {
  ...
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendStrictInstruction("与えられた内容をもとに、メール文面を作成してください。相手や状況に応じて、簡潔さ、丁寧さ、必要情報の過不足を意識してまとめてください。件名が必要そうであれば、件名案も併せて示してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendStrictInstruction("与えられた会話やメモをもとに、議事録ドラフトを作成してください。議題、主な論点、決定事項、未決事項、次のアクションが分かるように整理してください。必要であれば、参加者向けに読みやすい見出しや箇条書きへ整えてください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容から、課題一覧を整理してください。何が問題なのか、どこが未解決なのか、どの課題が他の課題に影響するのかが分かるように並べてください。必要であれば、課題ごとに簡潔な背景や優先度も添えてください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容から、他の課題や前提に依存している課題を抽出してください。どれが先行条件になっているのか、どの課題が他の進行を止めうるのかが分かるように整理してください。必要であれば、依存関係の向きも明示してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容について、実装計画を作成してください。調査、設計、実装、確認、テストなどの段階に分けて、どの順で進めるとよいかが分かるように整理してください。必要であれば、各段階の目的や確認ポイントも簡潔に添えてください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられたテーマについて、調査計画を作成してください。何を確認し、どの順で情報を集め、どこで判断材料を得るのかが分かるように整理してください。いきなり結論を出すのではなく、調査観点と調査手順が見えることを重視してください。")
  },
  {
...
- 自己判断の過程は必要に応じて記載して構いません。補足を記載する場合は、その理由や判断根拠が分かるようにしてください。`
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容について、根拠の弱い箇所を抽出してください。主張に対して理由が不足している点、裏付けが薄い点、断定が強すぎる点が分かるように整理してください。必要であれば、どのような追加根拠があると強くなるかも簡潔に示してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容に含まれる重要な用語について、用語集を作成してください。単語の羅列ではなく、それぞれが何を意味し、どの文脈で使われているのかが分かるように整理してください。必要に応じて、紛らわしい近い用語との違いも添えてください。")
  },
  {
  ...
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容をもとに FAQ を作成してください。利用者や読み手が疑問に思いそうな点を先回りして、質問と回答の形で整理してください。よくある質問として自然な粒度を意識し、回答は簡潔で実用的にまとめてください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容を、ナレッジベースや社内共有に向く形へ整理してください。単なる会話ログではなく、背景、要点、手順、注意点、FAQ など、後から参照しやすい構成を意識してまとめてください。必要であれば、見出し構成も提案してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた案や主張に対して、反対意見を代弁してください。無理に否定するのではなく、どこに懸念があり、どのような立場の人が反対しそうかが分かるように整理してください。必要であれば、反対意見が成立する条件も添えてください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("この案が失敗するとしたら、どのような理由や経路で失敗しうるかを考えてください。後から振り返るのではなく、事前に失敗パターンを想定するつもりで、見落としやすい要因を列挙してください。可能であれば、失敗を避けるための予防策も簡潔に添えてください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた主張や結論について、それに対する反証を考えてください。どの前提が崩れると主張が弱くなるか、どの事実や観点が反対材料になりうるかを整理してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた案や提案について、想定される反対意見を列挙してください。技術面、運用面、コスト面、関係者調整の面などから、相手が懸念しそうな点を広く整理してください。")
  },
  {
  ...
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容をセキュリティ視点で見てください。権限、入力、公開範囲、依存関係、情報漏えい、悪用可能性などの観点から、注意すべき点や追加確認が必要な点を整理してください。重大な懸念がある場合は、優先して指摘してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容を、出来事の発生順に時系列で整理してください。単に並べるだけでなく、どこで状況が変わったのか、どの時点が重要なのかが分かるようにまとめてください。必要であれば、各時点の意味や影響も簡潔に添えてください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容について、どのような順で意思決定や方針決定が行われたかを時系列で整理してください。どの時点で何が決まり、その判断が次にどう影響したのかが追えるようにまとめてください。必要であれば、未決のまま残っている点も併記してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容について、原因と結果を分けて整理してください。何が引き金となり、何がその結果として起こったのかを混ぜずに区別して示してください。必要であれば、直接原因と間接的な影響も分けて説明してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた問題や事象について、根本原因候補を列挙してください。表面的な現象の説明で止まらず、なぜそれが起きたのかを一段深く掘る観点で整理してください。まだ断定できない場合は、候補として複数並べて構いません。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容について、本当の問題設定が適切かを見直してください。いま解こうとしている問題が本質的な課題なのか、症状への対処に留まっていないか、問いそのものを再確認してください。必要であれば、別の問題設定案も示してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた問いや依頼を、より解きやすく、判断しやすい形に再定義してください。問いが広すぎる、曖昧すぎる、前提が混ざっている場合は、それを整理して、何を答えるべきかが明確になるように言い換えてください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた項目を、Must / Should / Could に分けて整理してください。必須、できれば必要、余裕があれば対応、のように優先度の違いが分かるように整理し、判断理由も簡潔に示してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた項目を、今やる / 後でやる / やらない に分けて整理してください。優先度だけでなく、なぜ今なのか、なぜ後なのか、なぜやらないのかが分かるように理由も簡潔に添えてください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた候補やタスクについて、優先順位を番号付きで並べてください。なぜその順番になるのかが分かるように、各項目の優先理由も簡潔に添えてください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた候補を優先順位づけするための観点を整理してください。重要度、緊急度、影響範囲、実装コスト、リスクなど、何を基準に順番を決めるべきかが分かるように整理してください。")
  },
  {
  ...
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた作業や議題について、対象範囲を明確化してください。何を含み、何を含まないのか、どこまでを今回扱うのかが分かるように整理してください。必要であれば、範囲が曖昧な点も指摘してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容について、今回やることとやらないことを分けて整理してください。対象外を明示することで作業の境界が見えるようにし、必要であれば、なぜ今回はやらないのかも簡潔に説明してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた作業や仕様について、スコープの境界を明確にしてください。どこからが対象で、どこからが対象外なのか、境界で判断に迷いやすい点も含めて整理してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた作業や計画について、スコープ肥大化のリスクを整理してください。どの要求や論点が範囲を広げやすいか、どこで歯止めをかけるべきかが分かるように整理してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた要求や案を、ユーザーストーリーの形に書き直してください。誰が、何をしたいのか、なぜそれが必要なのかが伝わるように整理してください。必要に応じて、利用者の価値や期待結果も明示してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容について、考えられるユースケースを列挙してください。誰がどの状況で使うのか、どのような流れがあるのかが分かるように、代表的な利用シナリオを整理してください。必要であれば、主要ケースと例外ケースを分けて示してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた要求や作業に対して、受け入れ条件を作成してください。何が満たされれば完了と見なせるのか、確認可能な形で整理してください。必要であれば、正常系、例外系、制約条件も含めて明示してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた作業について、完了条件を定義してください。単に「実装した」ではなく、確認、文書、テスト、共有なども含めて、どの状態なら完了と言えるのかが分かるように整理してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容について、テスト観点を列挙してください。どこを確認すれば抜け漏れが減るのかが分かるように、観点ベースで整理してください。必要であれば、機能、入力、境界、例外、依存のように分類して示してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容について、正常系 / 異常系 / 境界値 の観点で整理してください。確認すべきケースがどの枠に属するかを分けることで、テスト観点の抜け漏れを見つけやすくしてください。必要に応じて、各カテゴリの代表例も添えてください。")
  },
  {
  ...
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendStrictInstruction("与えられた内容を、Issue に書く文面へ変換してください。背景、問題、期待する対応、補足情報が整理され、後から読んでも何を扱う Issue なのかが分かるようにまとめてください。必要であれば、タイトル案も示してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容を、レビュー依頼文へ変換してください。何を見てほしいのか、どこに注意してほしいのか、レビュー相手が短時間で状況を把握できるように整理してください。必要であれば、前提や確認してほしい観点も添えてください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容について、不足情報を埋めるための質問を作ってください。いまの情報だけでは判断しにくい点や、解像度が足りない点を見つけて、優先度の高い質問から並べてください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容について、仕様確認のための質問を列挙してください。あいまいな仕様、未確定の振る舞い、境界条件、例外時の扱いなど、実装前に確認したほうがよい点を質問の形で整理してください。")
  },
  {
    id: "popular-bug-report-for-fix-request",
    label: "P1403-501: 不具合報告起票(修正担当者向け)",
    keywords: ["P1403-501", "bug report for developers", "fix-oriented bug report", "developer bug report", "reproduction steps", "environment details", "不具合報告", "修正担当向け", "再現手順", "発生環境", "バグ報告", "ふぐあいほうこく", "さいげんてじゅん", "はっせいかんきょう"],
    requiresCommitId: false,
...
${getMarkdownFenceInstruction()}`
  ...
  {
    id: "popular-bug-report-for-triage-request",
    label: "P1403-502: 不具合報告起票(管理用)",
    keywords: ["P1403-502", "bug triage report", "issue triage", "bug management", "priority owner", "不具合管理", "トリアージ", "優先度", "担当整理", "障害管理", "ふぐあいかんり", "ゆうせんど", "たんとう"],
    requiresCommitId: false,
...
${getMarkdownFenceInstruction()}`
  ...
  {
    id: "popular-bug-report-for-quality-analysis-request",
    label: "P1403-503: 不具合報告起票(品質分析用)",
    keywords: ["P1403-503", "quality analysis bug report", "defect analysis", "incident analysis", "frequency and cause", "品質分析", "不具合分析", "発生頻度", "原因分析", "欠陥分析", "ひんしつぶんせき", "はっせいひんど", "げんいんぶんせき"],
    requiresCommitId: false,
...
${getMarkdownFenceInstruction()}`
  ...
  {
    id: "popular-bug-report-for-customer-response-request",
    label: "P1403-504: 不具合報告起票(顧客対応用)",
    keywords: ["P1403-504", "customer-facing bug report", "customer incident response", "support response", "impact and workaround", "顧客対応", "利用者向け不具合報告", "影響範囲", "回避策", "案内文面", "こきゃくたいおう", "えいきょうはんい", "かいひさく"],
    requiresCommitId: false,
...
${getMarkdownFenceInstruction()}`
  ...
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容を、再利用しやすいテンプレートへ変換してください。毎回同じ構造で使えるように、変数部分と固定部分が分かる形で整理してください。必要であれば、利用時の簡単な補足も添えてください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容を、ひな形として使える骨格へ整えてください。まだ具体値が埋まっていなくても、どの順で何を書くのかが分かるように、見出しや項目を整理してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容を Mermaid で図示する前に、ノード候補、関係候補、流れ、階層を整理してください。いきなり記法へ落とすのではなく、何を図の要素として採用するのかが分かるように中間整理してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容を Graphviz DOT へ落とす前に、ノード候補とエッジ候補を整理してください。どの概念をノードにし、どの関係をエッジにするのかが分かるように、中間表現として整理してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容から、読み手が疑問に思いそうな点を増やして FAQ 化してください。すでに明示されている情報だけでなく、読み手が途中で引っかかりそうな点を先回りして質問化してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容について、誤解されやすいポイントから Q&A を作ってください。どこで誤読や思い込みが起きやすいかを見つけて、それを防ぐ質問と回答の形へ整理してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容を ADR 風に整理してください。背景、課題、選択肢、採用した判断、採用理由、影響を分けて、後から参照しやすい意思決定記録の形にまとめてください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた候補や判断について、採用した理由と不採用にした理由を記録してください。結論だけでなく、なぜそう判断したのかが後から追えるように、対比が見える形で整理してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容の中に、記述どうしの矛盾や不整合がないか確認してください。前後で言っていることが食い違っていないか、条件や結論が噛み合っているかを見て、問題があれば具体的に指摘してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容について、同じものを別の言葉で呼んでいないか、用語の揺れや表記の不整合を検出してください。読み手が混乱しそうな箇所があれば、統一候補も併せて示してください。")
  },
  {
  ...
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた初稿を改善してください。意図は保ちつつ、分かりやすさ、構成、表現の明確さを高める方向で見直してください。必要であれば、何をどう改善したかも簡潔に示してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容について、全体を書き直すのではなく、弱い点だけを改善してください。冗長な箇所、不明確な箇所、根拠が弱い箇所など、改善効果の高い部分に絞って修正してください。")
  },
  {
  ...
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容について、反対意見を持つ相手を説得するための文面を作ってください。押し切るのではなく、相手の懸念を踏まえたうえで、納得しやすい根拠と利点が伝わるように整理してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容を、合意形成しやすい形に整理してください。対立点だけでなく、共有できる前提や一致している目的も見えるようにし、どこから話を進めると合意しやすいかが分かるようにしてください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容について、実装コストを粗く見積もってください。厳密な工数ではなく、大 / 中 / 小 や 高 / 中 / 低 のような粒度で構いません。必要であれば、どの要因がコストを押し上げるのかも示してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容について、コストや工数に効きそうな要因を洗い出してください。実装難易度、依存関係、調査量、テスト負荷、確認相手の多さなど、何が重さの原因になりそうかを整理してください。")
  },
  {
  ...
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容について、ステークホルダーを整理してください。誰が影響を受けるのか、誰に確認が必要なのか、誰が意思決定に関わるのかが分かるように分類してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容について、説明相手ごとに関心を整理してください。どの相手が何を気にするのか、何を先に説明すると伝わりやすいのかが分かるようにまとめてください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容について、できることとできないことを分けて整理してください。期待を持たせすぎず、限界や前提も含めて、現実的にどこまで可能なのかが分かるように示してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容について、制約も含めて期待値を合わせる説明を作ってください。何がすぐできて、何が時間や条件を要し、何が難しいのかが分かるように、過剰期待を防ぐ形で整理してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容について、運用手順を整理してください。日常的にどう扱うか、どの順で何を確認し、どこで注意すべきかが分かるように、実務で使いやすい手順へまとめてください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容について、障害時の対応フローを整理してください。何を確認し、どの順で切り分け、どこで連絡し、どの状態で収束とみなすのかが分かるようにまとめてください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容について、観測すべき指標候補を出してください。何を見れば状態の良し悪しや異常の兆候が分かるのか、運用や改善に役立つ観点で整理してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容について、ログ / メトリクス / アラート の観点を整理してください。どこで記録し、何を数値で見て、どの条件で通知すべきかが分かるように、役割を分けて整理してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendStrictInstruction("与えられた内容をもとに、事象報告文面を作成してください。何が起きたのか、いつからか、影響は何か、現在どうなっているのかが短時間で伝わるように整理してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた事象や変更について、影響範囲を整理してください。誰に、どの機能に、どのデータに、どの程度の影響があるのかが分かるように整理してください。必要であれば、直接影響と間接影響を分けて示してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容について、脅威モデリングの入口となる観点を整理してください。何が守るべき資産で、どこに攻撃面があり、どの境界でリスクが高まりそうかが見えるように整理してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容について、攻撃面を洗い出してください。外部入力、公開インターフェース、権限境界、依存ライブラリなど、悪用されうる入口や露出箇所が分かるように整理してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容から、主要なエンティティを抽出してください。何がデータや概念の中心単位になるのか、どの対象を区別して扱う必要があるのかが分かるように整理してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容について、主要な要素どうしの関係性を整理してください。どれがどれに属し、依存し、参照し、連動するのかが分かるようにまとめてください。必要であれば、1対多や多対多のような関係も示してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた API 案について、入出力定義を整理してください。何を受け取り、何を返すのか、どの項目が必須でどの項目が任意かが分かるように整理してください。必要であれば、代表的な例も添えてください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた API 案について、エラーパターンを整理してください。どのような失敗がありうるか、どの条件でどのエラーを返すべきか、読み手や利用者が理解しやすい形で整理してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた画面や UI 案について、画面要素を列挙してください。何が表示され、何が入力され、何が操作対象になるのかが分かるように整理してください。必要であれば、要素の役割も簡潔に添えてください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた画面や機能について、操作フローを整理してください。利用者がどの順に見て、入力し、確認し、次へ進むのかが分かるようにまとめてください。必要であれば、分岐や例外時の流れも示してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた README について、改善観点を整理してください。何が不足しているか、どこが読みにくいか、どの情報が正本として弱いかなど、改善すると価値が高い点を列挙してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた文書について、構成を見直してください。情報の順番、見出しの切り方、正本情報と補助情報の分離など、読み手が追いやすくなる構成案を示してください。")
  },
  {
  ...
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendStrictInstruction("与えられた内容をもとに、利用者向けの告知文を作成してください。何が変わるのか、いつからか、利用者にどのような影響があるのかが短く分かるように整理してください。必要に応じて、対応が必要かどうかも明記してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendStrictInstruction("与えられた内容をもとに、メンテナンス告知文を作成してください。実施日時、影響範囲、利用者への影響、必要な注意点が分かるように整理してください。読み手がすぐ判断できるように、重要情報を先に示してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた仕様や説明について、未定義のケースを洗い出してください。通常ケースだけでなく、境界条件、例外条件、入力不足時、失敗時などで扱いが決まっていない点を見つけて整理してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた仕様について、例外時や異常時の仕様の穴を埋めるための質問を作ってください。失敗したらどうするか、入力が不正なときどう扱うか、想定外の状態でどう振る舞うかを確認できる質問を優先してください。")
  },
  {
  ...
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた依頼内容が曖昧な場合に、それを明確化してください。何を求めているのか、何が不足しているのか、どの範囲を対象としているのかが分かるように整理してください。必要であれば、確認すべき不足点も示してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた依頼内容を分解してください。大きな依頼を、そのままでは扱いにくい場合に、どのサブタスクや観点に分けると進めやすいかが分かるように整理してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた文章や依頼について、曖昧な表現を具体化してください。読み手によって解釈がぶれそうな語や表現を見つけて、必要に応じて言い換えや確認観点を示してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容について、解釈が割れそうな語や言い回しを洗い出してください。人によって意味がずれそうな語、範囲が曖昧な語、判断基準が不足している語を優先して整理してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容をもとに、会議アジェンダ案を作ってください。何を決めるのか、何を共有するのか、どの順で話すと効率的かが分かるように整理してください。必要であれば、各議題の目的も添えてください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた会議内容や議事メモから、会議後アクションを整理してください。誰が何をするのか、どこが未決なのか、次回までの宿題は何かが分かるように整理してください。")
  },
  {
  ...
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた長文を、スライド向けに再構成してください。段落をそのまま並べるのではなく、1 スライド 1 メッセージを意識して、箇条書きや図示しやすい単位へ整理してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた会話ログや議論ログを、説明資料向けに再構成してください。やり取りをそのまま並べるのではなく、背景、論点、結論、アクションが読み手に伝わる順に整理してください。")
  },
  {
  ...
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた旧版と新版について、何が変わったのかを整理してください。追加、削除、変更の観点で、読み手が差分をすぐ把握できるようにまとめてください。必要であれば、重要度の高い差分から並べてください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた変更点について、互換性への影響を整理してください。後方互換性が保たれるか、破壊的変更があるか、どの利用者や呼び出し側に影響しそうかが分かるように整理してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた変更や事象について、どのステークホルダーに何を通知するべきかを整理してください。利用者、運用担当、開発者、管理者などの相手ごとに、通知の必要性、伝えるべき内容、通知タイミングが分かるように整理してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendStrictInstruction("与えられた変更や事象について、相手ごとに伝え方を変えた通知文面を作ってください。ステークホルダーごとに関心や必要情報が異なることを前提に、共通情報と相手別補足が分かるように整理してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた目標や施策について、KPI 候補を定義してください。何を測れば成功や改善が判断できるのかが分かるように、指標名、意味、見方を整理してください。必要であれば、先行指標と結果指標も区別してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた評価指標について、どう測るのかを整理してください。必要なデータ、算出方法、集計単位、更新頻度、解釈上の注意点が分かるようにまとめてください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容について、主張と根拠を切り分けて整理してください。何を言っているのか、その裏付けは何か、推測に留まる部分はどこかが分かるように整理してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容について、根拠不足の箇所を整理してください。どの主張にどの種類の裏付けが不足しているのか、補強するとしたら何が必要かが分かるように整理してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた主張や提案について、各主張ごとに必要な根拠を列挙してください。事実、数値、実例、出典、利用者影響など、どの種類の裏付けが必要かが分かるように整理してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容について、出典や裏付けが必要な箇所を示してください。どの記述が断定的すぎるか、どの箇所に引用やデータが必要かが分かるように整理してください。")
  },
  {
  ...
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた機能や運用対象について、観測設計を整理してください。何をログに残すか、何をメトリクスとして持つか、どこでアラートを上げるか、どの異常を早く検知したいかが分かるように整理してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた機能や運用内容について、観測不足の箇所を洗い出してください。障害や劣化が起きても気づきにくい箇所、原因追跡が難しい箇所、ログや指標が足りない箇所を優先して整理してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendStrictInstruction("与えられた会話や作業状況について、次回再開用の要点を整理してください。何が決まっていて、何が未決で、次に何から始めるべきかが短く分かるように整理してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendStrictInstruction("与えられた作業内容について、再開時に最初に確認すべき事項を列挙してください。前提条件、未決事項、最新状態の確認ポイント、着手前に見直すべき点が分かるように整理してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容を、infographic の元になるテキストへ再構成してください。文章をそのまま残すのではなく、見出し、サブ見出し、要点、短い説明、強調したい数値やキーワード、矢印や囲みで表現しやすい関係性が分かるように整理してください。最終的に、描画AIや人間がそのまま infographic / 図解 / グラレコに起こしやすい粒度で出力してください。必要であれば、中央メッセージ、補助メッセージ、周辺要素に分けてください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容をもとに、infographic を描画させるための指示文を作成してください。レイアウト、主要見出し、視線誘導、強調語、アイコンやピクトグラムの候補、色数、図のトーン、文字量、避けるべき表現が分かるように整理してください。画像生成AIに渡しやすいよう、完成イメージが具体的に想像できる描画指示へまとめてください。必要であれば、英語ベースの image prompt と日本語の補足指示を分けて示してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容を infographic / 図解 / グラレコにする前提で、使えそうな視覚要素を抽出してください。たとえばアイコン、記号、囲み、関係線、対比構造、時系列、流れ、階層、強調に向く語句や数値などを整理し、何をどのように視覚化すると伝わりやすいかが分かるように示してください。必要であれば、各要素がどの情報に対応するかも併記してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容について、infographic / 図解 / グラレコ向けのレイアウト案を作成してください。どこにタイトルを置き、主要ブロックをどう配置し、どの順に視線が流れると理解しやすいかが分かるように整理してください。必要であれば、中央配置、左から右、上から下、タイムライン型、比較表型など複数の構図案を示し、それぞれの向き不向きも簡潔に添えてください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftInstruction("与えられた内容を、概念マップとして整理してください。主要な概念を抽出し、それぞれの関係を「原因と結果」「包含関係」「対立関係」「依存関係」「手段と目的」などの関係ラベルつきで表現できるようにまとめてください。単なる箇条書きではなく、どの概念が中心で、どの概念が周辺にあり、どうつながると理解しやすいかが分かるように整理してください。必要であれば、中心概念、主要ノード、補助ノード、接続ラベルに分けて出力してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftMarkdownFenceInstruction("与えられた内容をタイムライン化してください。まず通常の Markdown テキストで、日付または順序が分かる時系列一覧を整理してください。そのうえで、可能であれば Mermaid timeline も併記してください。出来事、判断、変更点、節目が追えるようにし、順序が不明確なものは推定せずに不明と明示してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftMarkdownFenceInstruction("与えられた内容をフローチャート化してください。まず通常の Markdown テキストで、主要な手順、分岐条件、例外分岐、終了条件が分かるように整理してください。そのうえで、可能であれば Mermaid flowchart も併記してください。処理順と分岐条件を混同せず、読み手が流れを追いやすい構成にしてください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendSoftMarkdownFenceInstruction("与えられた内容をマインドマップ化してください。まず通常の Markdown テキストで、中心テーマ、第1階層の論点、第2階層以下の補足が分かるように入れ子構造で整理してください。そのうえで、可能であれば Mermaid mindmap も併記してください。広げすぎて散漫にならないよう、中心テーマから主要論点が自然に追える構成を優先してください。")
  },
  {
  ...
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendStrictInstruction("与えられた内容について、法令違反のリスクがないか確認してください。何がどの法律に違反するかを断定するのではなく、違法性が問題になりそうな箇所、法令上の確認が必要そうな箇所、解釈を誤ると危険そうな箇所を整理してください。必要に応じて、どの観点で専門確認が必要かも示してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendStrictInstruction("与えられた内容について、規約違反やルール違反のリスクがないか確認してください。法令違反とまでは言えなくても、利用規約、運用ルール、投稿ガイドライン、社内ルールなどに照らして問題になりそうな箇所を整理してください。どの種別のルールに抵触しそうかが分かるように示してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendStrictInstruction("与えられた内容について、権利侵害のリスクがないか確認してください。著作権、商標、肖像、プライバシー、利用許諾の観点から、問題になりそうな箇所を整理してください。何がどの権利に関係しそうか、追加確認が必要そうな点はどこかが分かるように示してください。")
  },
  {
    ...
    requiresCommitId: false,
    buildBody: () => appendStrictInstruction("与えられた内容について、個人情報や機密情報のリスクがないか確認してください。本人特定につながる情報、公開範囲に注意が必要な情報、内部情報として扱うべき内容、伏せるべき情報が含まれていないかを整理してください。必要に応じて、どの情報を伏せる、ぼかす、削るべきかの観点も示してください。")
  }
];
