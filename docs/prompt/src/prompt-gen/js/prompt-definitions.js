...
const promptDefinitions = [
    {
        ...
        buildBody: (commitId) => commitId
            ? `対象コミット ${commitId} における変更内容について、PRタイトルとPR本文を markdown テキスト形式で作文してください。

# Context

- Git作業の指定コミットの変更内容をもとに、プルリクエストに記載するテキストを作成したい。

# Instructions

- 指定された変更内容に基づいて、PRタイトルとPR本文を Markdown テキスト形式で作成してください。
...
これから、生成AIのみならず人間も理解できるよう、前半は人間向けの説明、後半は生成AI向けの本文、必要に応じて例を示す補助セクションも持つ、構造化された生成AIプロンプトの作文を支援してください。この依頼は、完成したプロンプトを一方的に出力することだけを目的とするものではなく、必要に応じて構成案、不足点、改善案を示しながら、最終的に実際に使えるプロンプト本文まで組み立てていくためのものです。特に、ここで生成する対象プロンプト自体が、いきなり完成版を吐く一発型ではなく、材料整理、主題整理、確認、構成合意を経て進む伴走型の振る舞いを既定とするようにしてください。
...
- 生成対象のプロンプト自体も、利用者に対して伴走型に振る舞う前提
...
- どのような制約や条件があるのか
- どのような結果を期待しているのか
- 利用者が最初から主題や依頼内容をきれいに定義できているとは限らず、断片メモや雑な説明から出発すること
...
- 初回応答では完成版をいきなり出さず、まず材料整理、主題整理、目的確認、必要なら不足情報の確認から始めること
- ユーザーが即完成を望んだ場合のみ、一括出力してよいこと
- 利用者の入力が断片的な場合は、まず何を作りたい話なのか、何を主題に据えるべきかを整理すること
- まず構成案を示すこと
- 次に不足点や曖昧な点を整理すること
- 必要ならユーザーに確認すること
...
- 生成対象のプロンプトには、利用者へ一度に質問を詰め込みすぎず、焦点を決めるのに必要な事項から順に確認する振る舞いを書き込んでよいこと
- 生成対象のプロンプトには、内容整理の途中で仮の subject や主題を導出してよいこと
- 生成AIに何をさせたいのか
- 生成AIに何をさせたくないのか
- どの観点や優先順位で判断してほしいのか
- どのような制約を守ってほしいのか
...
- 生成対象のプロンプトも、一発型ではなく伴走型を既定動作とする
...
- 伴走型プロンプトを作る場合は、初回応答で材料提示を促す例、主題整理の例、構成合意の例を「# Examples」に含めてよい
`
    },
    {
        ...
        requiresCommitId: false,
        outputMarkdown: true,
        buildBody: () => `これは、Qiita 向けの技術記事を作るための構造化プロンプトです。
...
- ユーザーは最初から記事テーマをきれいに定義できているとは限らず、断片的なメモ、箇条書き、コミット、背景説明、悩みだけを渡す場合があります
- そのため、いきなり完成記事を書き始めるのではなく、材料を見ながら記事の主題、対象読者、記事価値、扱う範囲、記事外へ分離すべき論点を伴走的に整理する前提で扱います
...
- まず、ユーザーから記事材料を受け取る前提で動く
- 初回応答では、いきなり完成記事を書かず、まず何を材料として出してもらうとよいかを簡潔に案内する
- ユーザーから材料が渡されたら、その内容から記事の核になりそうな主題を整理し、仮の subject を導出する
- 導出した仮 subject は、ユーザー入力の単純転記ではなく、「この記事は結局何の話か」が一読で分かる短い主題として整理する
- 次に、その材料で 1 本の Qiita 記事として焦点が保てるかを判断する
- 焦点がぼやける兆候がある場合は、本文を書き切る前に、どの単位で記事を分けるとよいかを先に提案する
- 基本の進め方は、材料整理、主題導出、対象読者の明確化、記事の価値整理、構成案、必要なら補足質問、本文ドラフト、推敲、完成版、の順に伴走する
- 質問は一度に詰め込みすぎず、記事の焦点を決めるうえで不足している情報から優先して聞く
- 情報が十分に集まるまでは、見出し案、論点整理、書くこと / 書かないことの整理を優先し、完成稿の即出しを既定動作にしない
- ユーザーが明示的に完成稿を求めた場合のみ、その時点の情報で不足を明示しつつ、Qiita 投稿用の記事タイトルと記事本文を Markdown 形式で作文してよい
...
- 技術的に正しそうでも、ユーザーが渡していない事実を勝手に補完しない
- 初回から完成稿を押しつけず、伴走相手として段階的に整理する
- 仮 subject は、内容整理の結果として導出するものであり、最初から固定値として扱わない

# Examples

- 初回の伴走開始例:
  - まず記事材料をください。箇条書きメモ、やったこと、困ったこと、比較した案、コミット、スクラップのどれでも構いません
  - 材料をもらったら、まず「この記事は何の話として立てるか」を整理し、Qiita 向けの主題と構成案を先に出します
- 見出し構成の例:
  - はじめに
  - 背景や課題
  - 何をしたか / 何ができるか
...
- 分割提案の例:
  ...
  - まず本記事では概要と判断理由に絞り、詳細は関連記事へ分ける案を提案します`
    },
    {
        ...
        requiresCommitId: false,
        outputMarkdown: true,
        buildBody: () => `これは、note 向けの記事を作るための構造化プロンプトです。
...
- ユーザーは最初から記事テーマをきれいに定義できているとは限らず、断片的なメモ、出来事の羅列、感想の断片、背景説明だけを渡す場合があります
- そのため、いきなり完成記事を書き始めるのではなく、材料を見ながら記事の主題、読み物としての核、感情や実感の軸、本文に残すこと、記事外へ逃がすことを伴走的に整理する前提で扱います
...
- 初回応答では、いきなり完成記事を書かず、まず何を材料として出してもらうとよいかを簡潔に案内する
- ユーザーから材料が渡されたら、その内容から記事の核になりそうな主題を整理し、仮の subject を導出する
- 導出した仮 subject は、ユーザー入力の単純転記ではなく、「この記事は結局どんな実感や出来事の話か」が一読で分かる短い主題として整理する
- 次に、その材料で 1 本の note 記事として自然な読み物になるかを判断する
- 技術解説や仕様説明が主役になりすぎる場合は、そのまま書き切る前に、Qiita 記事など別記事へ分離したほうがよい部分を先に示してよい
- 基本の進め方は、材料整理、主題導出、読み物としての核の明確化、構成案、必要なら補足質問、本文ドラフト、推敲、完成版、の順に伴走する
- 質問は一度に詰め込みすぎず、読み物としての焦点を決めるうえで不足している情報から優先して聞く
- 情報が十分に集まるまでは、何を書きたいのか、どこに実感の核があるのか、何を本文から外すべきかの整理を優先し、完成稿の即出しを既定動作にしない
- ユーザーが明示的に完成稿を求めた場合のみ、その時点の情報で不足を明示しつつ、note 投稿用の記事タイトルと記事本文を Markdown 形式で作文してよい
- 自然な読み物として書く
...
- モデル名や製品名を主役にしすぎない
- 技術的に正しそうでも、ユーザーが渡していない事実や感情を勝手に補完しない
- 初回から完成稿を押しつけず、伴走相手として段階的に整理する
- 仮 subject は、内容整理の結果として導出するものであり、最初から固定値として扱わない
...
- 初回の伴走開始例:
  - まず記事材料をください。出来事のメモ、気になった瞬間、感情の揺れ、断片的な下書き、何を書きたいか曖昧な状態の説明でも構いません
  - 材料をもらったら、まず「この記事はどんな実感や出来事の話として立てるか」を整理し、note 向けの主題と構成案を先に出します
- 見出し構成の例:
  - はじめに
  - きっかけや背景
  - 印象的だったことや考えたこと
...
- 技術詳細の扱いの例:
  ...
  - note 本文では、体験の意味や気づきが伝わる範囲にとどめる`
    },
    {
        ...
        requiresCommitId: false,
        outputMarkdown: true,
        buildBody: () => `これは、技術作文や技術説明の文章を、伴走しながらわかりやすく整理するための構造化プロンプトです。

これから、技術作文や技術説明の文章を人間にも生成AIにも意図が伝わりやすい形で構造化して整理してください。前半では背景や狙いを説明し、後半では実際に生成AIへ渡す指示、守るべきルール、必要に応じて補助的な例を分けて記述してください。単に本文を一発で書き直すことだけを目的とせず、必要に応じて話題ラベルの導出、分離判断、確認、整理方針の提示を挟みながら、最終的にわかりやすい説明文へ組み立てていくためのプロンプトとして扱ってください。

# Context

- このプロンプトは、技術作文や技術説明の文章を整理するためのものです
- 利用者は最初から「何の話か」を短く言い表せているとは限らず、長い説明、断片メモ、混線した文章、途中まで書いた本文をそのまま渡す場合があります
- 技術者が技術作文で陥りがちなパターンとして、話題を先に固定しない、本文を書きながら話題が増える、読者に「わからない」と言われて詳細説明を足してさらに膨らませる、という流れがあります
- このパターンを避けるために、いきなり本文を膨らませるのではなく、まずタイトル・見出し・話題ラベルを導出し、何の話なのかを固定してから整理することを重視します
- 良い整理の基準は、読者が「何の話か」をすぐつかめることです
- 良い整理の基準は、話題ラベルに直接対応する内容へ絞られ、脱線した話題が無理に混ざっていないことです
- 良い整理の基準は、情報を増やしすぎず、一文を短くし、言い換え、順番調整、不要部分の削減によって理解しやすくなっていることです
- 説明が伝わりにくい場合でも、直ちに情報量不足と決めつけず、用語の硬さ、話の順番、一文の詰め込みすぎ、前提知識の要求しすぎを先に見直す前提で扱います

# Instructions

- まず、利用者から元の文章や断片メモを受け取る前提で動く
- 初回応答では、いきなり本文を書き直さず、まず元の文章または説明したい内容を提示してもらうよう簡潔に案内する
- 入力が渡されたら、まずその文章が「何の話なのか」を一言で言い直し、タイトル・見出し・話題ラベルの候補として提示する
- 話題ラベル候補が複数ありうる場合は、どれがもっとも適切かを利用者と短く会話して決める
- 話題ラベルが固まるまでは、本文の全面書き換えを既定動作にしない
- 話題ラベルが固まったら、そのラベルに直接対応する内容に絞って整理方針を立てる
- 話題ラベルに入りきらない内容、別の話題、脱線している内容は、無理に一つへ詰め込まず、分離したほうがよい候補として示す
- 読者に「わからない」と言われても、すぐ詳細説明を足して補強しない
- まず、一文を短くする、言い換える、話を分ける、順番を入れ替える、不要な前提や枝葉を削る、という方向で改善する
- 詳細追加は最後の手段にする
- 必要に応じて、話題ラベル候補、何の話かの一文要約、元の文章の問題点、分離したほうがよい話題、整理後の本文案、という順で段階的に整理してよい
- 質問は一度に詰め込みすぎず、話題ラベルを決めるうえで必要な確認から優先して聞く
- 利用者が明示的に本文案まで欲しいときのみ、整理後の本文案までまとめて提示してよい

# Rules

- いきなり本文を書き始めず、まず話題ラベルを固定する
- 話題ラベルや一文要約は、利用者入力の単純な繰り返しではなく、何の話かが一読でわかる形に整理する
- 仮の話題ラベルや仮説を、利用者確認前の確定事項のように扱わない
- 話題が複数混ざっている場合は、無理に一つへ詰め込まず分離提案を行う
- 読みにくさを、ただちに情報量不足の問題だと決めつけない
- 詳細説明の追加より先に、短文化、言い換え、順番調整、削減、分離を優先する
- 用語が硬すぎないか、話の順番が悪くないか、一文に複数のことを詰め込みすぎていないか、前提知識を求めすぎていないかを先に疑う
- 質問過多にならず、伴走相手として必要最小限の確認で前に進める
- 利用者が渡していない意図や前提を勝手に補完しない

# Examples

- 初回の伴走開始例:
  - まず元の文章、または説明したい内容の断片をください。長文のままでも、途中までの下書きでも構いません
  - 受け取ったら、まず「何の話か」を一言で整理し、話題ラベル候補と整理方針を先に出します
- 話題ラベル候補の例:
  - 候補1: Vitest で DOM テストを追加するときのハマりどころ
  - 候補2: DOM テスト導入時に見落としやすい前提整理
- 分離提案の例:
  - これは「エラーの原因説明」と「最終的な運用ルール」が混ざっています。1 本に詰め込まず、まず原因説明に絞ったほうが読みやすいです
- 整理順の例:
  - 話題ラベル候補
  - 何の話かの一文要約
  - 元の文章の問題点
...
作業開始前に、この制約を理解した旨を短く確認してから進めてください。`
    ...
    {
        id: "minimal-scope-change-request",
        label: "A751: 要求達成に必要な範囲だけを変更",
        keywords: ["A751", "751", "minimal scope change", "scoped change", "focused change", "minimal impact", "必要最小限", "必要な範囲だけ", "要求達成", "影響範囲", "無関係な変更をしない", "リファクタしない", "整形しない", "命名変更しない", "周辺最適化しない", "ひつようさいしょうげん", "ようきゅうたっせい", "えいきょうはんい", "りふぁくたしない", "せいけいしない", "めいめいへんこうしない"],
        requiresCommitId: false,
        buildBody: () => "今回の要求を満たすために必要な範囲だけを変更してください。無関係なリファクタ、整形、命名変更、周辺最適化は行わないでください。差分の小ささ自体ではなく、意図と影響範囲を限定することを優先してください。"
    },
    {
        id: "canonical-resource-receiving-request",
        label: "A752: 正本受領中は OK のみ回答",
        keywords: ["A752", "752", "canonical source receiving", "resource receiving", "receive only", "ok only", "handover intake", "正本受領", "受領中", "OKのみ", "受け取り専念", "中身に反応しない", "解説しない", "指摘しない", "渡し終えたら反応", "せいほんじゅりょう", "じゅりょうちゅう", "おーけーのみ", "うけとりせんねん", "なかみにはんのうしない"],
        requiresCommitId: false,
        buildBody: () => "これから正本となるソースコード、markdown、関連資料を順次渡します。私が受領完了を明示するまでは、中身の要約、解説、評価、改善提案、不具合指摘、質問は行わず、各回の応答は厳密に `OK` のみとしてください。まだ処理や分析は開始せず、受け取りに専念してください。私が「これで渡し終えた」と伝えてから、はじめて内容に基づく応答を開始してください。"
    },
    {
        id: "canonical-resource-return-request",
        label: "A753: 正本返却時は全文を出力",
        keywords: ["A753", "753", "canonical source return", "return full text", "full content output", "no diff", "changed files only", "正本返却", "全文出力", "差分ではなく全文", "変更したものだけ", "未変更は不要", "省略しない", "完成後全文", "せいほんへんきゃく", "ぜんぶんしゅつりょく", "さぶんではなくぜんぶん", "みへんこうはふよう", "しょうりゃくしない"],
        requiresCommitId: false,
        outputMarkdown: true,
        buildBody: () => "正本を返却する際は、未変更のものは出力せず、変更した正本だけを漏れなく出力してください。各対象について、差分や抜粋ではなく完成後の全文を提示してください。`...` などの省略記法は使わないでください。"
    }
];
