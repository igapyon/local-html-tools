...
- **git-pseudo-squash.html**: reset --soft で疑似的に1コミット化するコマンドを生成します。複数行のコミットメッセージに対応し、各行が -m オプションで git commit に渡されます。HTML属性はシングルクォートを使用してシェル実行時のクォート解析問題を回避しています。
