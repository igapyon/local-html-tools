...
![local-html-tools overview](docs/assets/local-html-tools-ogp.png)

GitHub Pages 公開版: <https://igapyon.github.io/local-html-tools/>

ブラウザでそのまま開いて使える、ローカル完結の Static Web App コレクションです。サーバ不要で、各ツールは単一 HTML として動作します。
...
- OSS として配布するソースには、原則として著作権表示と `SPDX-License-Identifier` を含む短いソースヘッダーを付与する
- 対象はまず手編集する正本ファイルとし、生成物はビルド結果として自然にヘッダーが残る状態を基本とする
- `*.html` の生成物を `*-src.html` から作る領域では、正本の `*-src.html` にヘッダーを置く
- `src/` 配下の `*.js` `*.ts` `*.css` にも同様にヘッダーを付与してよい
- 生成後の単一HTMLで同一ヘッダーが過剰に重複する場合は、ビルド時に先頭付近へ集約する方針を検討する
- ライセンス本文はルートの `LICENSE` を正本とする
