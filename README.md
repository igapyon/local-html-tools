...
- `local-html-tools` の UI 改修は `lht`（`lht-cmn` の Web Components）を中心に進める
- Material Web は基盤として利用しつつ、画面側では `lht-*` を使って宣言的に構築する
- これにより次を狙う:
  - 作業効率向上（同じ UI パターンの再実装を減らす）
  - ソース記述量削減（画面側を「何を置くか」に集中）
  - 挙動統一（`required` / help 表示 / focus / event の差異を集約）
  - 生成AI編集性向上（変更点の局所化と副作用抑制）
- 実装手順の原則:
  - まず `lht-cmn` に部品を追加・調整する
  - 次に各 `*-src.html` を `lht-*` 利用へ置換する
  - 最後にビルドで単一HTML化して配布物へ反映する

### LHT完結方針（将来計画）

- `lht-*` を画面側の唯一の公開UI層として扱う（画面HTMLで `md-*` の直接利用を増やさない）
- `lht-cmn/js/components.js` は次の優先順位で実装する:
  - 1. Material Web コンポーネントが利用可能なら内部で利用する
  - 2. 利用不可なら `lht-cmn` 内のフォールバック実装へ自動で切り替える
- `lht-cmn/css/components.css` を最終的な正本CSSとし、将来的に `md3/` 依存を段階的に縮退する
- `md3/` は当面リファレンスとして併存可能だが、実運用の見た目・挙動の責務は `lht-cmn` 側へ寄せる
